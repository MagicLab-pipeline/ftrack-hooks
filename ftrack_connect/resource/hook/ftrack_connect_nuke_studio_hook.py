    ...
    def _discoverApplications(self):
        ...
        if sys.platform == 'darwin':
            ...
            applications.extend(self._searchFilesystem(
                expression=prefix + ['Nuke.*', 'Nuke\d.+.exe'],
                versionExpression=version_expression,
                label='Nuke Studio (Non-commercial)',
                variant='{version}',
                applicationIdentifier='nuke_studio_(Non-commercial)-{version}',
                icon='nuke_studio',
                launchArguments=['--studio', '--nc']
            ))
...
def register(session, **kw):
    '''Register hook for ftrack connect legacy plugins.'''
