...
class ApplicationStore(ftrack_connect.application.ApplicationStore):
    ...
    def _discoverApplications(self):
        ...
        applications = []
        import ft_utils
        prefix = None #ft_utils.get_app_folder_path()


        if sys.platform == 'darwin':
            if not prefix:
                prefix = ['/', 'Applications']
            print "\n>>> ftrack_connect_nuke_hook.py:: reading Nuke app from: {}".format(prefix)
            ...
            '''
            # Add Nuke no commercial as a separate application
            applications.extend(self._searchFilesystem(
                expression=prefix + ['Nuke.*', 'Nuke\d.+.exe'],
                versionExpression=nuke_version_expression,
                launchArguments=['--nc'],
                label='Nuke(No Commercial)',
                variant='{version}',
                applicationIdentifier='nuke_(nc)-{version}',
                icon='nuke'
            ))
            '''
