...
class VLCAction(object):
    """Launch vlc action."""

    identifier = "vlc-launch-action"

    def __init__(self, applicationStore, launcher):

        super(VLCAction, self).__init__()
    ...
    def launch(self, event):
        """Callback method for vlc action."""
...
class ApplicationStore(ftrack_connect.application.ApplicationStore):
    ...
    def _discoverApplications(self):
        ...
        """
        applications = []
        icon = os.environ.get('PIPELINE_IMAGES')
        icon += "\\vlc-icon.png"
        #print ">>>>>>>>> icon vlc: {}".format(icon)
        if sys.platform == "darwin":
            pass

        elif sys.platform == "win32":
            applications.extend(self._searchFilesystem(
                expression=["C:\\", "Program Files*", "VideoLAN", "VLC",
                            "vlc.exe"],
                label="VLC",
                applicationIdentifier="vlc",
                icon=icon
            ))
...
def register(registry, **kw):
    ...
    action = VLCAction(applicationStore, launcher)
    action.register()
